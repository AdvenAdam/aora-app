...
export const cekBoorkmark = (bookmarks, userId) => {
    return bookmarks.some((item) => item.$id === userId);
};

export const updateBookmark = async (bookmarks, idVideo) => {
    try {
        const bookmark = await databases.updateDocument(config.databaseId, config.videoCollectionId, idVideo, {
            bookmarks
        });
        return bookmark;
    } catch (error) {
...
export const getAllBookmarks = async (user) => {
    try {
        const bookmarks = getAllPosts();
